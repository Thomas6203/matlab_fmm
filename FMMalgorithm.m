global max_particles level
max_particles = 64;
level = 0;
mask_quadtree = initialize_quadtree(mask);
...
function quadtree = initialize_quadtree(particles, boundary)
...
U_list = struct('node', '', 'list', [] ); %%有任一共同的邊界者(一個小方塊四周的邊界)
...
function pre_order_traversal = pre_order_traversal(root_node)
    ...
    if root_node(1).children ~= {}
        ...
        %如何對於一個節點，讀取或列出所有與其相同層級的節點 ??
        for i = 1:4
            for j = 1:4
                if (root_node(i).level == root_node(1).level)...
                    && root_node(i).boundary(j) == root_node(1).boundary(i)
